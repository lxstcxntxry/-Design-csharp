...
public static class Utils
{
    public static int GetNumericInput()
    {
        while (true)
        {
            ...
            catch (System.FormatException) // https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/exceptions/creating-and-throwing-exceptions
			{
    ...
    private static Genre GetGenreInput() // incapsulation
    {
    ...
    private static string GetISBNInput()
    {
        ...
        var regex = new Regex(@"^\d{3}-\d{1}-\d{2}-\d{6}-\d$"); // regexp
        while (true)
        {
            string input = Console.ReadLine();
    ...
    public static async Task PreWorkLoad() // https://habr.com/ru/articles/248505/
								     // –епозитории представл€ют коллекции.  ак вы храните
								     // эти коллекции Ч это просто деталь реализации.
								     // я хочу внести €сность в этот вопрос.
								     //
								     // –епозиторий Ч это коллекци€.
								     //  оллекци€, котора€ содержит сущности и может фильтровать
								     // и возвращать результат обратно в зависимости от требований
								     // вашего приложени€. √де и как он хранит
								     // эти объекты €вл€етс€ ƒ≈“јЋ№ё –≈јЋ»«ј÷»».
	{
